...
public class SeedChooser : MonoBehaviour
{
	...
	public void ResetThis()
	{
		...
		for (int i = 0; i < Pages.Count; i++)
		{
			...
			for (int j = 0; j < componentsInChildren.Length; j++)
			{
				if (componentsInChildren[j].CardPlantType != PlantType.Nope)
				{
					componentsInChildren[j].isForCreate = false;
					componentsInChildren[j].IsUnLock = GameManager.Instance.LocalPlayerSave.UnlockedPlants.Contains(componentsInChildren[j].CardPlantType);
					if (LV.Instance.LvSpStates.Contains(LVSpState.LastStand) && new List<PlantType>
					{
						PlantType.SunFlower,
						PlantType.SunShroom,
						PlantType.TwinSunflower,
						PlantType.Heronsbill
					}.Contains(componentsInChildren[j].CardPlantType))
					{
						componentsInChildren[j].IsChoosed = true;
					}
				}
			}
		}
	...
	public void Prepare()
	{
		...
		SelectPrepare selectPrepare = new SelectPrepare();
		selectPrepare.isPrepare = isPrepare;
		SocketClient.Instance.SelectPrepare(selectPrepare);
		BattlePlayerList.Instance.UpdateState(GameManager.Instance.LocalPlayerSave.playerName, selectPrepare.isPrepare);
	}
	...
	private IEnumerator DoMove(float targetPosX, bool isOut)
	{
		...
		Vector2 dir = (vector - base.transform.position).normalized * 4000f;
		if (targetPosX > base.transform.position.x)
		{
			while (targetPosX > base.transform.position.x)
			{
				yield return null;
				rectTransform.Translate(dir * Time.deltaTime);
			}
		}
		else
		{
