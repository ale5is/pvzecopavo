...
public class SocketClient : MonoBehaviour
{
    ...
    void ConnectAndReceive(Socket socket, IPAddress ip, int port, PlayerInfo info)
    {
        ...
        {
            if (!isClosing) UnityMain(() => Debug.Log("Error de conexión: " + e));
        }
        catch (ObjectDisposedException)
        {
            if (!isClosing) UnityMain(() => Debug.Log("El socket se ha cerrado."));
        }
        catch (Exception e)
        {
            if (!isClosing) UnityMain(() => Debug.LogError(e));
        }
    }
    ...
    void ReceiveLoop(Socket socket)
    {
        ...
        try
        {
            while (!isClosing && GetSocket() == socket)
            {
                ...
                {
                    if (!isClosing)
                        UnityMain(() => Debug.LogError(new Exception("Conexión perdida", e)));
                    break;
                }
                ...
                {
                    UnityMain(() => Debug.Log("Los datos recibidos son demasiado grandes."));
                    break;
                }
                ...
                while (pendingCount - offset >= 4)
                {
                    ...
                    {
                        UnityMain(() => Debug.Log("Se recibió un paquete de datos no válido."));
                        return;
                    }
    ...
    void ProcessMessage(byte[] data)
    {
        ...
        switch (data[0])
        {
            case 0: ProcessConnection(data[1], s); break;
            case 1: ProcessGame(data[1], s); break;
            case 2: ProcessSpawn(data[1], s); break;
            case 3: ProcessWorld(data[1], s); break;
            case 4: ProcessCommand(data[1], s); break;
        }
    }
    ...
    void ProcessConnection(byte type, string s)
    {
        ...
        if (type == 3)
        {
            ...
            ReConnect.Instance.LoadPlayerList(info.names ?? new List<string>());
        }
    }

    void ProcessPendingOnlinePlayerInfo()
    {
        if (!hasPendingOnlinePlayerInfo || pendingOnlinePlayerInfo == null)
            return;

        var info = pendingOnlinePlayerInfo;
        var players = info.players ?? new List<PlayerInfo>();

        if (!pendingPlayerListDone && PlayerList.Instance != null)
        {
            PlayerList.Instance.UpdatePlayerList(info.HostPlayer, players);
            pendingPlayerListDone = true;
        }

        if (!pendingBattlePlayerListDone && BattlePlayerList.Instance != null)
        {
            BattlePlayerList.Instance.UpdatePlayerList(info.HostPlayer, players);
            pendingBattlePlayerListDone = true;
        }

        if (pendingPlayerListDone && pendingBattlePlayerListDone)
        {
            pendingOnlinePlayerInfo = null;
            hasPendingOnlinePlayerInfo = false;
        }
    ...
    void ProcessGame(byte type, string s)
    {
        switch (type)
        {
            case 0:
                ...
                if (load == null || LVManager.Instance == null) return;
            ...
            case 8:
                ...
                if (over != null && LV.Instance != null && LVManager.Instance != null)
                {
                    if (LV.Instance.CurrLVType == LVType.PvP)
                        LVManager.Instance.PvPGameOver(over.pos, over.isRedFail);
                    else
            ...
            case 11:
                ...
                if (list != null)
                    SpectatorList.Instance?.ClientSynList(list.names ?? new List<string>());
                break;

            case 12:
                var add = JsonUtility.FromJson<AddCardBag>(s);
                if (add != null)
                    SeedBank.Instance?.AddCards(add.CardTypes, true);
                break;
    ...
    void ProcessSpawn(byte type, string s)
    {
        switch (type)
        {
            case 0:
                ...
                if (plant == null || PlantManager.Instance == null ||
                    SeedBank.Instance == null || MapManager.Instance == null)
                    return;

                var obj = PlantManager.Instance.GetNewPlant(plant.plantType);
                if (obj == null)
                    return;
                ...
                ReversePvP(ref plant.GridPos, plant.PlacePlayer);
                obj.OnlineId = plant.OnlineId;

                var grid = MapManager.Instance.GetGridByWorldPos(plant.GridPos);
                if (grid != null)
                    SeedBank.Instance.PlantConfirm(obj, grid, -1, plant.SPcode, plant.PlacePlayer);
                break;
            ...
            case 4:
                ...
                if (save != null && cd.name == save.playerName)
                {
                    if (cd.OK)
                        SeedBank.Instance?.PlantFailClearCD(cd.CardId);
                }
                else
            ...
            case 5:
                ...
                if (shovel != null)
                    BattlePlayerList.Instance?.PreviewShovel(shovel.PlayerName, shovel.GridPos, shovel.isShow);
                break;

            case 6:
                var tool = JsonUtility.FromJson<ToolApply>(s);
                if (tool != null)
                    BattlePlayerList.Instance?.PlayShovelAnimation(tool.GridPos, tool.Sound, tool.User);
                break;
        }
    }
    ...
    void ProcessWorld(byte type, string s)
    {
        switch (type)
        {
            ...
            case 1:
                ...
                var ggrid = grave != null ? MapManager.Instance?.GetGridByWorldPos(grave.MapPos) : null;
                if (ggrid != null)
                    ggrid.ClientSynGrave(grave.Type, grave.isHave);
                break;

            case 2:
                CreatePuddle(JsonUtility.FromJson<PuddleSpawn>(s));
                break;

            case 3:
                var light = JsonUtility.FromJson<LightingSpawn>(s);
                var lgrid = light != null ? MapManager.Instance?.GetGridByWorldPos(light.Pos) : null;
                if (lgrid != null)
            ...
            case 4:
                ...
                if (map != null)
                    MapManager.Instance?.GetCurrMap(map.mapPos)?.SynMap(map);
                break;
            ...
            case 7:
                SynGrid(JsonUtility.FromJson<SynGrid>(s));
                break;

            case 8:
                var booty = JsonUtility.FromJson<SynBooty>(s);
                if (booty != null && LVManager.Instance?.InGame == true)
                {
                    if (booty.isSpawn)
    ...
    void CreatePuddle(PuddleSpawn spawn)
    {
        ...
        var puddle = Instantiate(GameManager.Instance.GameConf.Puddle).GetComponent<Puddle>();
        if (puddle == null)
            return;

        puddle.CreateInit(grids, spawn.InitPos, spawn.OnlineId);
        MapManager.Instance.puddles.Add(puddle);
    }

    void SynGrid(SynGrid data)
    {
        if (data == null || MapManager.Instance == null || LV.Instance == null)
            return;
        ...
        MapManager.Instance.GetGridByWorldPos(data.GridPos)?.ClientSynState(data);
    }
    ...
    void ProcessCommand(byte type, string s)
    {
        switch (type)
        {
            ...
            case 2:
                ...
                if (chat != null)
                    ChatInput.Instance?.AddMessage("Jugador" + chat.PlayerName + "Susurro:" + chat.content, new Color32(123, 123, 123, 255));
                break;
            ...
            case 3:
                ...
                if (PlantManager.Instance != null) PlantManager.Instance.PlantInvincible = cmd.Pinv;
                ...
                if (SkyManager.Instance != null) SkyManager.Instance.DayLightCycle = cmd.DLiCy;
                if (PlayerManager.Instance != null) PlayerManager.Instance.SunInfinite = cmd.SnInf;
                if (SeedBank.Instance != null) SeedBank.Instance.isNoCD = cmd.CdCle;
                if (LvItemManager.Instance != null) LvItemManager.Instance.VaseAlwaysLight = cmd.VaseXray;
                break;
            ...
            case 5:
                ...
                if (time == null || SkyManager.Instance == null)
                    return;

                if (time.time >= 10000)
                    SkyManager.Instance.DirectSetTime(time.time - 10000, true);
                else
                    SkyManager.Instance.Time = time.time;
                break;

            case 6:
                var achievement = JsonUtility.FromJson<GetAcvment>(s);
                if (achievement != null)
                    AcvmentManager.Instance?.GetAchievement(achievement.acv);
                break;
    ...
    void SendMsg(string content, byte type1, byte type2, Socket socket = null)
    {
        ...
        try
        {
            ...
            {
                int n = target.Send(buffer, sent, buffer.Length - sent, SocketFlags.None);
                if (n <= 0)
    ...
    IEnumerator WaitLog()
    {
        ...
        if (GameManager.Instance?.isOnline == true || connectionResponseReceived)
        {
            WaitConnect = null;
            yield break;
        }
    ...
    public void ChangeMap(PlayerMap map) => SendMsg(JsonUtility.ToJson(map), 1, 3);
    public void SelectCard(SelectCard card) => SendMsg(JsonUtility.ToJson(card), 1, 4);
    public void SelectPrepare(SelectPrepare prepare) => SendMsg(JsonUtility.ToJson(prepare), 1, 5);
    public void ApplyTool(ToolApply apply) => SendMsg(JsonUtility.ToJson(apply), 1, 1);
    ...
    public void ClickedSun(ClickedSun sun) => SendMsg(JsonUtility.ToJson(sun), 1, 2);
    public void ApplyPlacePlant(PlantSpawn spawn) => SendMsg(JsonUtility.ToJson(spawn), 1, 0);
    public void ApplyPlaceZombie(ZombieSpawnApply spawn) => SendMsg(JsonUtility.ToJson(spawn), 1, 10);
    public void ApplyPlacePreview(PlantPreview spawn) => SendMsg(JsonUtility.ToJson(spawn), 1, 7);
    public void ApplyShovelPreview(ShovelPreview spawn) => SendMsg(JsonUtility.ToJson(spawn), 1, 9);
    public void ApplyZombiePreview(ZombiePreview spawn) => SendMsg(JsonUtility.ToJson(spawn), 1, 11);
    public void ApplyJoinTeam(JoinTeamApply spawn) => SendMsg(JsonUtility.ToJson(spawn), 1, 12);
    public void ApplyJoinSpect(JoinSpecApply spawn) => SendMsg(JsonUtility.ToJson(spawn), 1, 13);
    public void SendSynBag(SynItem syn) => SendMsg(JsonUtility.ToJson(syn), 1, 6);
    public void SendSlotMBag(SlotMchBag bag) => SendMsg(JsonUtility.ToJson(bag), 1, 14);
    ...
    public void SynItem(SynItem syn)
    {
        ...
        if (syn.Type == SynItemType.AllItem || syn.Type == SynItemType.Zombie)
        {
            if (ZombieManager.Instance != null)
            {
                var zombies = new List<ZombieBase>(ZombieManager.Instance.GetAllZombies());
                var hyp = ZombieManager.Instance.GetAllHypZombies();
