...
public class GreenStar : BulletBase
{
	public Light2D light2;

	private Glowstarfruit Starfruit;
	...
	public void Init(int attackValue, Vector2 pos, int currLine, Vector2 dirc, Glowstarfruit starfruit, bool isCheck, bool isHyp)
	{
		isTurned = starfruit.TurnedChange;
		isHypno = isHyp;
		Starfruit = starfruit;
	...
	protected override void UpdateThis()
	{
		...
		if (isCheck && Starfruit != null)
		{
			Starfruit.CheckZombieResult(isTurned);
		}
		if (!isCheck)
		{
			...
			{
				base.CurrGrid.CurrPlantBase.Dizzy(Random.Range(1, 3));
			}
			HitEff();
	...
	protected override void TriggerEnter(Collider2D collision)
	{
		...
		if (collision.tag == "Zombie")
		{
			...
			if ((componentInParent.CurrLine == CurrLine || CurrLine == -1) && ((componentInParent.isHypno && isHypno) || (!componentInParent.isHypno && !isHypno)))
			{
				...
				if (!isCheck)
				{
					...
					if (Random.Range(0, 7) > 5)
					{
						componentInParent.Dizzy(Random.Range(1, 3));
					}
					HitEff();
				}
			}
		...
		if (!collision.transform.GetComponent<MapWall>().IsPass(Dirction))
		{
			if (!isCheck)
			{
				if (Random.Range(0, 3) == 0)
	...
	{
		PoolManager.Instance.PushObj(GameManager.Instance.GameConf.GreenStar, base.gameObject);
	}
