...
public class Sun : MonoBehaviour
{
    ...
    public bool CanGet
    {
        get
        {
            if (!IsClicked)
            {
                return sphereCollider != null && sphereCollider.enabled;
            }
    ...
    private void Update()
    {
        if (isFromSky &&
            !IsClicked &&
            !(base.transform.position.y <= downTargetPosY))
        {
    ...
    public void CollectSun(bool synClient = false)
    {
        ...
        if (IsSunType() && !SeedBank.Instance.NeedSummonSun)
        {
            ...
            !SeedBank.Instance.NeedSummonMoon)
        {
            vector = base.transform.position;
        }
        ...
        if (LV.Instance.CurrLVType == LVType.PvP &&
            (OwnerPlayer == null ||
                !PvPSelector.Instance.IsSameTeam(OwnerPlayer)) &&
            OwnerPlayer != null)
        {
            vector = base.transform.position;
        }
        ...
        if (MapManager.Instance.GetCurrMap(base.transform.position) ==
    ...
    public void TombFlyToPosDes(Vector3 pos, UnityAction action)
    {
        if (!IsClicked &&
            ...
            (GameManager.Instance.isClient ||
                !(MapManager.Instance.GetCurrMap(base.transform.position) == null)))
        {
            isTombAbsorb = true;
    ...
    private void SunSound()
    {
        if (IsSunType())
        {
            if (Random.Range(0, 2) > 0)
            {
                AudioManager.Instance.PlayEFAudio(
                    GameManager.Instance.AudioConf.SunClick,
            ...
            {
                AudioManager.Instance.PlayEFAudio(
                    GameManager.Instance.AudioConf.SunClickHigh,
                    base.transform.position
                );
            }
        }
        ...
        {
            AudioManager.Instance.PlayEFAudio(
                GameManager.Instance.AudioConf.MoonClick,
                base.transform.position
            );
        }
    }
    ...
    public void InitForSky(
        float downTargetPosY,
        Vector3 SpawnPos,
        SunType type)
    {
        InitForAll(
        ...
        if (!GameManager.Instance.isClient &&
            SkyManager.Instance.SunAutoCollect &&
            sunType != SunType.Red)
        {
            CollectSun();
        }
    ...
    private IEnumerator DoJump()
    {
        ...
        while (
            (double)base.transform.position.y >=
            (double)startPos.y - 0.2)
        {
            yield return null;
    ...
    public void DestroySun()
    {
        ...
        PoolManager.Instance.PushObj(
            GameManager.Instance.GameConf.Sun,
            base.gameObject
        );
    }
}