...
public class CustomYard : MapBase
{
	private Vector2Int mapGridNum;
	...
	public SpriteRenderer FenceREnder;

	public SpriteRenderer FenceShadowREnder;

	public SpriteRenderer FenceBackREnder;

	public List<Sprite> HouseSprites = new List<Sprite>();

	public List<Sprite> FenceSprites = new List<Sprite>();

	public List<Sprite> FenceShadowSprites = new List<Sprite>();

	public List<Sprite> FenceBackSprites = new List<Sprite>();
	...
	public override Vector2Int MapGridNum => mapGridNum;
	...
	{
		puddleColor = new Color(1f, 1f, 1f);
	...
	protected override void TimeChange(int time)
	{
		...
		{
			tiles[j].SetColor(new Color(puddleColor.r, puddleColor.g, puddleColor.b, 1f));
		}
		for (int k = 0; k < SnowsList.Count; k++)
		{
			SnowsList[k].TimeChange();
			SnowsList[k].ChangeColor(SnowScence.color);
		}
		for (int l = 0; l < ScenceItem.Count; l++)
		{
			ScenceItem[l].color = new Color(puddleColor.r, puddleColor.g, puddleColor.b, 1f);
		}
	...
	private void SpawnSnow(bool Fade)
	{
		...
		for (int i = 0; i < gridList.Count; i++)
		{
			...
			component.CreateInit(gridList[i], Fade);
			component.transform.SetParent(base.transform);
			if (gridList[i].Point.y == 0)
			{
				if (gridList[i].Point.x == 0 || gridList[i].Point.x == 7)
				{
					component.BigSnow.transform.localScale = new Vector3(1f, 0.94f);
				}
				if (gridList[i].Point.x == 8)
				{
					component.BigSnow.transform.localScale = new Vector3(1f, 0.88f);
				}
			}
			gridList[i].snow = component;
	...
	public override void CustomMapEditInit(int vertical, int horizontal, CustomMapSave save)
	{
		mapGridNum = new Vector2Int(horizontal, vertical);
		float num = 7.85f / (float)vertical;
		float num2 = 14.62f / (float)horizontal;
		Vector3 vector = base.transform.position + new Vector3(-7.65f + num2 / 2f, 3.15f - num / 2f);
		for (int i = 0; i < vertical; i++)
		{
			for (int j = 0; j < horizontal; j++)
			{
				GridList.Add(new Grid(new Vector2Int(j, i), vector + new Vector3(num2 * (float)j, (0f - num) * (float)i, 0f)));
			}
		}
		float y = 2.9f / (float)vertical;
		float x = 6.2f / (float)horizontal;
		for (int k = 0; k < GridList.Count; k++)
		{
			CustomTile component = Object.Instantiate(GameManager.Instance.GameConf.CustomTile).GetComponent<CustomTile>();
			component.transform.position = GridList[k].Position;
			component.transform.SetParent(TilesGroup);
			component.transform.localScale = new Vector3(x, y);
			tiles.Add(component);
			if (GridList[k].Point.x < save.HorizontalNum && GridList[k].Point.y < save.VerticalNum)
			{
				int num3 = GridList[k].Point.x + GridList[k].Point.y * save.HorizontalNum;
				if (save.tileTypes.Count > num3)
				{
					component.SetType(save.tileTypes[num3], GridList[k]);
				}
				else
				{
					component.SetType(TileType.Grass, GridList[k]);
				}
			}
			else
			{
		...
		if (save.HouseType > 0 && save.HouseType < HouseSprites.Count)
		{
			HouseREnder.sprite = HouseSprites[save.HouseType];
		}
		else
		{
			HouseREnder.sprite = HouseSprites[0];
		}
		if (save.FenceType > 0 && save.FenceType < FenceSprites.Count)
		{
			FenceREnder.sprite = FenceSprites[save.FenceType];
			FenceShadowREnder.sprite = FenceShadowSprites[save.FenceType];
		}
		else
		{
			FenceREnder.sprite = FenceSprites[0];
			FenceShadowREnder.sprite = FenceShadowSprites[0];
		}
		if (save.FenceBackType > 0 && save.FenceBackType < HouseSprites.Count)
		{
			FenceBackREnder.sprite = FenceBackSprites[save.FenceBackType];
		}
		else
		{
			FenceBackREnder.sprite = FenceBackSprites[0];
		}
	}

	public override int ChangeDecoration(int decorType)
	{
		switch (decorType)
		{
		case 0:
		{
			int num3 = HouseSprites.IndexOf(HouseREnder.sprite) + 1;
			if (num3 < HouseSprites.Count)
			{
				HouseREnder.sprite = HouseSprites[num3];
			}
			else
			{
				HouseREnder.sprite = HouseSprites[0];
			}
			return HouseSprites.IndexOf(HouseREnder.sprite);
		}
		case 1:
		{
			int num2 = FenceSprites.IndexOf(FenceREnder.sprite) + 1;
			if (num2 < FenceSprites.Count)
			{
				FenceREnder.sprite = FenceSprites[num2];
				FenceShadowREnder.sprite = FenceShadowSprites[num2];
			}
			else
			{
				FenceREnder.sprite = FenceSprites[0];
			...
			return FenceSprites.IndexOf(FenceREnder.sprite);
		}
		case 2:
		{
			int num = FenceBackSprites.IndexOf(FenceBackREnder.sprite) + 1;
			if (num < FenceBackSprites.Count)
			{
				FenceBackREnder.sprite = FenceBackSprites[num];
			}
			else
			{
				FenceBackREnder.sprite = FenceBackSprites[0];
			}
			return FenceBackSprites.IndexOf(FenceBackREnder.sprite);
		}
		default:
			return base.ChangeDecoration(decorType);
		}
	}
}
